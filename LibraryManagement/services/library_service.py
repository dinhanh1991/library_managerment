...
class LibraryService:
    ...
    def add_book(self, book):
        ...
        if not book_id:
            return False
        if book.quantity < 0 or book.publish_year < 0:
            return False
    ...
    def get_overdue_borrowers(self, current_date=None):
        ...
        borrowers = self.borrower_repo.load_borrowers()
        return [borrower for borrower in borrowers if self._is_overdue(borrower, current_date)]
    ...
    def book_borrow(self, borrower):
        ...
        if any(item.borrower_id == borrower_id for item in borrowers):
            return False
        if any(item.borrower_id == borrower_id and item.book_id == book_id for item in self.borrow_queue.items):
            return False
    ...
    def return_book(self, borrower):
        ...
        self.borrow_queue.items = [
            item for item in self.borrow_queue.items
            if not (item.borrower_id == borrower_id and item.book_id == book_id)
        ]
        self.queue_repo.save_queue(self.borrow_queue.items)
        self._refresh_structures()
        return True
